...
export class FLAMSServer {
  ...
  async apiSettings(): Promise<flams.Settings | undefined> {
    const ret = await this.postRequest<
      {},
      [flams.Settings, any, any] | undefined
    >("api/settings", {});
    if (ret) {
  ...
  async apiLogin(username: string, password: string): Promise<void> {
    await this.postRequest("api/login", {
      username: username,
      password: password,
    });
  }
  ...
  async backendGroupEntries(
    ...
    return await this.postRequest("api/backend/group_entries", {
      in: in_entry,
    });
  }
  ...
  async backendArchiveEntries(
    ...
    return await this.postRequest("api/backend/archive_entries", {
      archive: archive,
      path: in_path,
    });
  }
  ...
  private async getRequest<TRequest extends Record<string, unknown>, TResponse>(
    endpoint: string,
    request: TRequest
