...
import * as FTML from "./ftml-viewer/ftml_viewer.js";
...
export type ExerciseOptions = {graded:[string,FTML.ExerciseFeedback][]} | {withSolutions:[string,FTML.Solutions][]} | {onEvent:((r:FTML.ExerciseResponse) => void)};
...
export const FTMLDocument: React.FC<{ 
  ...
  useEffect(() => {
    ...
    const on_section_start = onSectionBegin? (uri:string) => {
      ...
      if (r) { return (e:HTMLDivElement,o:FTML.LeptosContext) => 
        addTunnel(e,r,o)}
    }:undefined;
    ...
    const handle = FTML.render_document(mountRef.current, opts,
      on_section_start,
      on_section_end,
      context
    );
...
export const FTMLFragment: React.FC<{ opt: FragmentOptions, ex?:ExerciseOptions }> = ({ opt,ex }) => {
  const mountRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    if (ex) {
      if ("graded" in ex) {
        handle = FTML.render_fragment(mountRef.current, opts,
          context,
